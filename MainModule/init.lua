...
local DataStoreEngine = require(Server.DataStoreEngine)
local ServerFunctions = require(Server.Functions)
local _warn = warn
local function warn(...)
	_warn("[Rudimentary]:",...)
end
...
APIFunctions = {
	...
	["requestAuth"] = function(plr, message)
		...
		RF.Name = require(ReplicatedStorage:WaitForChild("Rudimentary"):WaitForChild("Shared").Key)(100)
...
local function handlePlayer(player)
	task.spawn(function()
		...
		if (tick() - Start) <= maxtime then
			...
			script.Client.ClientLoader:Clone().Parent = player.PlayerGui:WaitForChild("RudimentaryUi")
		end
		local Ban = nil
...
local function setupAdmin(Config, Requirer)
	...
	mainTable.DataStore = DataStoreEngine.new(Config.DataStoreName or Settings.DataStoreName,DataStoreEngine.DataStoreTypes.Normal)
				...
				if not table.find(NonEditableSettings, index) then 
					task.spawn(function()
						if mainTable.DataStore:GetAsync(index) == nil then
							...
							res.result:Connect(function(saved) 
								if saved then
	...
	for _, Command:ModuleScript in pairs(script.Commands:GetDescendants()) do
	...
	if Requirer:FindFirstChild("Themes") and #Requirer.Themes:GetChildren() > 0 then
		for _, theme in Requirer.Themes:GetChildren() do
			theme:Clone().Parent = script.Themes
		end
	end
	for _, theme in script.Themes:GetChildren() do
	...
	for _, item in script.Themes:GetChildren() do
